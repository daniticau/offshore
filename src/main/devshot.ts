...
export function setupDevshot(): void {
  ...
  /** Stay out of the way: no stealing the front, no stealing the keyboard. */
  const quiet = process.env['OFFSHORE_QUIET'] === '1'
  ...
  async function run(): Promise<void> {
    ...
    /*
     * Ensure the view has painted before capturing.
     *
     * By default that means putting the window frontmost, which is fine on a
     * machine nobody is using — and thoroughly rude on one somebody is. With
     * OFFSHORE_QUIET=1 the window is parked off the side of the display
     * instead: it stays shown, so it keeps rendering and capturePage still has
     * a real surface to read, but it never comes to the front and never takes
     * the keyboard away from whatever you were typing into.
     */
    if (quiet) {
...
function setupTestFlows(): void {
  ...
  if (!flow) return
  /** Run off the side of the display instead of on top of whatever you're doing. */
  const quietFlow = process.env['OFFSHORE_QUIET'] === '1'
  let failures = 0
  ...
  async function runFlow(): Promise<void> {
    ...
    if (flow === 'chrome') {
       ...
       */
      /*
       * OFFSHORE_QUIET=1 parks the window off the side of the display rather
       * than hauling it to the front. It still has to take focus — this section
       * is about the cursor being in the address bar, and there is no way to
       * test that without it — but off-screen it does so without covering
